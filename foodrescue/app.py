...
def stack_food():
    with get_db() as db:
        items = db.execute("SELECT id, foodname, quantity, email FROM DONATION ORDER BY id DESC").fetchall()
...
def order_food():
    ...
    with get_db() as db:
        donation_row = db.execute(
            "SELECT foodname, email FROM DONATION WHERE id = ?", (donation_id,)
        ).fetchone()
        ...
        db.execute("DELETE FROM DONATION WHERE id = ?", (donation_id,))

    send_email_notifications(donation_row["foodname"], donation_row["email"], session["email"])
    flash(f"{donation_row['foodname']} has been ordered! A notification email has been sent, if email is configured.")
    return redirect("/stackFood")
...
def send_email_notifications(foodname, donor_email, recipient_email):
    ...
    donor_message = (
        f"Hello,\n\nYour donated item '{foodname}' has been ordered by a recipient. "
        f"Thank you for your donation!\n\nFood Rescue"
    )
    recipient_message = (
        f"Hello,\n\nYou've successfully ordered '{foodname}'. "
        f"Please coordinate with the donor for pickup/delivery details.\n\nFood Rescue"
    )
    ...
    try:
        with smtplib.SMTP("smtp.gmail.com", 587) as server:
            ...
            for recipient, body in ((donor_email, donor_message), (recipient_email, recipient_message)):
                msg = EmailMessage()
...
def chat_bot():
    ...
    if query:
        if not SPOONACULAR_API_KEY:
            error = "Recipe search isn't configured yet — set SPOONACULAR_API_KEY in your environment."
        else:
            try:
                resp = requests.get(
                    "https://api.spoonacular.com/recipes/findByIngredients",
                    params={"ingredients": query, "number": 6, "apiKey": SPOONACULAR_API_KEY},
                    timeout=8,
                )
                resp.raise_for_status()
                recipes = resp.json()
            except requests.RequestException as exc:
                error = f"Couldn't reach the recipe service right now ({exc})."

    return render_template("chat-bot.html", query=query, recipes=recipes, error=error)
